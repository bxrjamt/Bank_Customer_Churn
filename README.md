...
| Fase | Qué se hace |
|------|------------|
| 1️⃣ **Carga y separación train-test** | - Cargar los datos <br> - Separar entre conjunto de entrenamiento y test para evaluar los modelos más adelante |
| 2️⃣ **EDA (Exploración de datos)** | - Describir estadísticas básicas para entender el comportamiento general de las variables <br> - Visualizar variables y su distribución <br> - Analizar la interacción entre variables para detectar patrones importantes <br> - Estudiar la correlación entre variables para identificar relaciones relevantes |
| 3️⃣ **Preprocesamiento de datos** | - Eliminar variables irrelevantes o redundantes <br> - Tratar valores nulos: <br> &nbsp;&nbsp;- Imputación basada en lógicas de negocio <br> &nbsp;&nbsp;- Imputación con valores estimados usando técnicas estadísticas <br> - Aplicar one-hot encoding para variables categóricas y dejar los datos listos para los modelos |
| 4️⃣ **Balanceo de clases** | - Ajustar el dataset para que los modelos no se sesguen hacia la clase mayoritaria <br> - Preparar el test para la evaluación final |
| 5️⃣ **Entrenamiento y evaluación de modelos** | - Probar regresión logística como modelo base <br> - Entrenar XGBoost para mejorar la predicción del churn |
| 6️⃣ **Interpretación de variables en el modelo** | - Analizar qué variables influyen más en la decisión de los clientes de quedarse o irse <br> - Extraer conclusiones que permitan diseñar estrategias de retención |
| 7️⃣ **Generación de comunicaciones con GPT** | - Usar IA generativa para crear mensajes personalizados que animen a los clientes a quedarse |
| 8️⃣ **Envío de correo** | - Ejecutar la campaña de retención con los clientes identificados como en riesgo |
